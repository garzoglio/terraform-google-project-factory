...
class OrgPermissions:
    ...
    def __init__(self, org_id, shared_vpc=False, parent=False):
        ...
        Args:
            ...
            shared_vpc (bool): Whether parent permissions should be checked
        """
        self.org_id = org_id
        self.shared_vpc = shared_vpc
        self.parent = parent

        self.permissions = self.ALL_PERMISSIONS.copy()
...
def argparser():
    ...
    parser.add_argument(
        ...
        help='The project ID of the seed project where the service account '
             'is defined'
    )
    parser.add_argument(
        '--shared_vpc',
...
def validators_for(opts):
    """
    Given a set of CLI options, determine which preconditions we need
    to check and generate corresponding validators.
    """
    validators = [
        SeedProjectServices(opts.project_id),
        BillingAccount(opts.billing_account),
    ]

    if opts.folder_id:
        org_is_parent = False
        folder_validator = FolderPermissions(opts.folder_id, parent=True)
        validators.append(folder_validator)
    else:
        org_is_parent = True
...
def main(argv):
    ...
    validators = validators_for(opts)
